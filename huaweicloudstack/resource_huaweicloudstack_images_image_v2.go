...
func resourceImagesImageV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack image client: %s", err)
	}
...
func resourceImagesImageV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("metadata", img.Metadata); err != nil {
		return fmt.Errorf("[DEBUG] Error saving metadata to state for HuaweiCloudStack image (%s): %s", d.Id(), err)
	}
	...
	if err := d.Set("tags", img.Tags); err != nil {
		return fmt.Errorf("[DEBUG] Error saving tags to state for HuaweiCloudStack image (%s): %s", d.Id(), err)
	}
...
var DiskFormats = [4]string{"vhd", "qcow2", "raw", "zvhd"}
